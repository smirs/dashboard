...
# One-Time Costs
Buying_Cost = st.slider('Listing Price', 100000, 250000, 150000, 500)

# Initial Annual Costs [changes with the property]
HOA = st.slider('Monthly HOA Fee', 0, 500, 250, 50)
Maintenance_Annual_Cost = 12*HOA #12000

# Initial Annual Gains 
Rent = st.slider('Estimated Monthly Rate', 1000, 3000, 1500, 100)
...
ROI10 = round(Total_Gain(20) / Total_Cost(20)*10 , 1)
ROI20 = round(Total_Gain(20) / Total_Cost(20)*10 , 1)
...
y = x
fig = sns.scatterplot(x=[1, 2, 3], y=[1, 3, 2])
fig.set_xlim(1, 100)
fig.set_ylim(0, 1)
...
fig.fill_between(x, 0, thresh_ok, where= y<thresh_ok,
                color='red', alpha=0.5, transform=fig.get_xaxis_transform())

plt.axvline(thresh_great, 0 ,1)

fig.fill_between(x, 0, thresh_great, where= (y>thresh_ok) & (y<thresh_great),
                color='yellow', alpha=0.5, transform=fig.get_xaxis_transform())
